    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#f9f8f4] px-4">
            <div className="w-[340px] space-y-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-[#f55b7a]">Select your preferences</h3>
            ...
            {isVeg ? "VEG" : "NON-VEG"}
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center justify-start px-3 py-2 rounded-md text-sm font-medium transition ${
                                    selected ? "bg-orange-400 text-white" : "bg-orange-100 text-gray-800"
                                }`}
                            >
                                <div className={`w-3 h-3 rounded-full mr-2 ${selected ? "bg-teal-600" : "bg-gray-300"}`} />
                                {cat}
                            </button>
                        );
                    })}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {meatDishes.map((meat, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-3 h-3 rounded-full mr-2 ${selected ? "bg-red-600" : "bg-teal-600"}`} />
                                    {meat}
                                </button>
                            );
                        })}
                ...
                <div className="flex items-center justify-between px-4 text-sm font-medium text-gray-700">
                    ...
                    >
                        ...
                        />
                        <span className="absolute left-2 text-xs text-white font-bold">{notifications ? "ON" : "OFF"}</span>
                    </div>
                </div>

                <CustomButton ref={button} color="secondary" className="w-full mt-8">
                    Continue
                </CustomButton>
            </div>
        </div>
